...
fn receive_ambient_air_tempterature_updates(broker_uri: &str, topic: &str) -> Result<(), String> {
    ...
    let mut _subscribe_response = client.subscribe(topic, MQTT_QOS).map_err(|err| format!("Failed to subscribe to topic {topic} due to '{:?}'", err));
    ...
    for msg in receiver.iter() {
        if let Some(msg) = msg {
            ...
            if client.reconnect().is_ok() {
                _subscribe_response = client.subscribe(topic, MQTT_QOS).map_err(|err| format!("Failed to subscribe to topic {topic} due to '{:?}'", err));
            } else {
                break;
            }
        }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _receive_result = receive_ambient_air_tempterature_updates(&broker_uri, &topic).map_err(|err| Status::internal(format!("{:?}", err)));
