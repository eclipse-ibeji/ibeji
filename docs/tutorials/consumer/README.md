...
- [2. Discover a Digital Twin Provider with the In-Vehicle Digital Twin Service](#2-discover-a-digital-twin-provider-with-the-in-vehicle-digital-twin-service)
  - [2.1 Rust Sample Discovery of a Digital Twin Providerr](#21-rust-sample-discovery-of-a-digital-twin-provider)
...
To interact with your digital twin provider, the digital twin consumer must use the same communication protocol and understand the interface contract. The programming languages of the digital twin consumer and the digital twin provider does not need to match.
...
1. Choose the programming language for implementing your digital twin consumer. The programming language you choose should contain support for gRPC and using the same communication protocol that your digital twin provider. gRPC is required to communicate with the In-Vehicle Digital Twin Service. This will be described further in [2. Discover a Digital Twin Provider with the In-Vehicle Digital Twin Service](#2-discover-a-digital-twin-provider-with-the-in-vehicle-digital-twin-service). For instance, if your digital twin provider uses MQTT, then you should select a programming language that supports MQTT and gRPC. This includes languages like Rust, Python, Java, C++, Go, etc.
...
This section uses the [sample digital twin provider interface](../provider/README.md#sample-digital-twin-provider-interface) that is defined in a protobuf file, and covers calling `Get` and `Invoke` operations that are defined in the [sample digital twin provider](../../../samples/tutorial/provider/src/provider_impl.rs). This sample digital twin provider was used in the [Tutorial: Create a Digital Twin Provider](../provider/README.md#rust-sample-implementation-of-the-sample-interface).
...
Your digital twin consumer will need to discover your digital twin provider using the In-Vehicle Digital Twin Service before the digital twin consumer can interact with the provider.

To communicate with the In-Vehicle Digital Twin Service, your digital twin consumer must use gRPC and the gRPC client must conform to the [In-Vehicle Digital Twin's interface](../../../interfaces/invehicle_digital_twin/v1/invehicle_digital_twin.proto).
...
1. For each in-vehicle signal or command for which you are discovering a provider, you can refer to the signal or command using the code from your in-vehicle digital twin model.

## 3. Add Managed Subscribe to Digital Twin Consumer

In the previous [Tutorial: Create a Digital Twin Provider](../provider/README.md), the [Managed Subscribe module was introduced for providers](../provider/README.md#3-add-managed-subscribe-to-digital-twin-provider).
